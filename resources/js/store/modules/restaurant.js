...
const getters = {
    ...
    getVegMenuItems: state =>
        state.menu_items.filter(items => items.type === "veg"),
    getNonVegMenuItems: state =>
        state.menu_items.filter(items => items.type === "non-veg"),
    getVegRecommendedMenuItems: state =>
        state.recommended_menu_items.filter(items => items.type === "non-veg")
};
...
const actions = {
    async trendingRestaurants({ commit }) {
        return axios
    ...
    async premiumRestaurants({ commit }) {
        return axios
    ...
    restaurantCounts({ commit, dispatch }) {
        axios
            .get("/api/restaurants/trending")
            .then(res => res.data)
            .then(res => {
    ...
    async recommendedMenu({ commit }, id) {
        return axios
            .get(`/api/menu/${id}`)
            .then(res => res.data)
            .then(res => {
    ...
    async vegRecommendeMenuItems({ commit }, id) {
        return axios
            .get(`/api/menu/${id}/veg`)
            .then(res => res.data)
            .then(res => {
