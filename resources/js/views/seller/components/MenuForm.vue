...
export default {
    ...
    computed: {
        ...
        menu_item_total() {
            return (
                parseInt(this.menu_item.price) +
                parseInt((this.menu_item.price * this.menu_item.gst) / 100) +
                parseInt(this.menu_item.delivery_charge)
            );
        }
    },
    methods: {
        createMenuItem() {
            ...
            this.$store
                .dispatch("sellerMenu/createMenu", data)
                .then(() => {
                    this.formError = false;
                    this.submitted = false;
