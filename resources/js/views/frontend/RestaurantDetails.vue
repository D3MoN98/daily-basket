            ...
            <div class="food_tab_otr side_chkbox">
              <div id="food_tab">
                  ...
                  <div class="modal_chkbox tab_chk_bx">
                    <div class="chk_inn">
                      <div class="form-check custom_chkbx">
                        ...
                        <label class="form-check-label">Only Veg</label>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="resp-tabs-container">
                  <div>
                    <RecommendedMenu
                      :isRecommendedMenuItemLoaded="isRecommendedMenuItemLoaded"
                      :recommended_menu_items="recommended_menu_items"
                    />
                  </div>
                  <div>
                    <Menu :isMenuItemLoaded="isMenuItemLoaded" :menu_items="menu_items" />
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  created () {
    this.$store.dispatch('restaurant/restaurant', this.$route.params.slug).then(() => {
      this.$store.dispatch('restaurant/menu', this.restaurant.id).then(() => {
        this.isMenuItemLoaded = true;
      });
      this.$store.dispatch('restaurant/recommendedMenu', this.restaurant.id).then(() => {
  ...
  methods: {
    onlyVeg ($e) {
      ...
      if ($e.target.checked) {
        this.$store.dispatch('restaurant/vegMenuItems', this.restaurant.id).then(() => { this.isMenuItemLoaded = true; });
        this.$store.dispatch('restaurant/vegRecommendeMenuItems', this.restaurant.id).then(() => { this.isRecommendedMenuItemLoaded = true; });
        ...
        this.$store.dispatch('restaurant/menu', this.restaurant.id).then(() => { this.isMenuItemLoaded = true; });
        this.$store.dispatch('restaurant/recommendedMenu', this.restaurant.id).then(() => { this.isRecommendedMenuItemLoaded = true; });
      }
    },
  },
};
