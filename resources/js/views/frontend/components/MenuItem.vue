  ...
  <li>
    ...
    <div class="food_details">
      ...
      <div class="food_qty">
        ...
        >Add</button>
        <div v-else class="qty_number animate__animated animate__fadeIn">
          <span class="minus" @click="addToCart(menu_item.id, -1, cart_item.rowId)">-</span>
          <input type="text" :value="cart_item.qty" disabled />
          <span class="plus" @click="addToCart(menu_item.id, 1, cart_item.rowId)">+</span>
        </div>
...
export default {
  ...
  computed: {
    ...
    cart_item () {
      return this.cart_items.find(((item) => item.id === this.menu_item.id), this.menu_item.id);
    },
  ...
  methods: {
    addToCart (id, qty, rowId = null) {
      ...
      const newQty = this.cart_item === undefined ? qty : this.cart_item.qty + qty;
